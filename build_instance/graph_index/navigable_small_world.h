...
public:
    ...
    void SearchTopKonDevice(float* queries, int num_of_topk, int* &results, int num_of_query_points, int num_of_candidates){
        ...
        cout << "Query speed: " << (float)num_of_query_points/((float)std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count()/1000000) << " queries per second" << endl;
    ...
    void Establishment(int num_of_initial_neighbors, int num_of_candidates){
        ...
        std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
        cout << "Running time: " << (float)std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count()/1000000 << " seconds" << endl;
    }
